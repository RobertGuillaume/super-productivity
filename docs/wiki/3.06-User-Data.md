...
Experimental Solid backend uploads use the same application-model snapshot
boundary. **Upload current data to Solid** captures the current local snapshot,
validates it, and writes supported model data to the user's Solid Pod: tasks,
...
Local REST API access token. Solid storage is experimental and separate from
the normal sync-provider pipeline.
