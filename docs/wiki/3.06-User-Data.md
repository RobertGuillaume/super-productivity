...
Solid backend uploads use the same application-model snapshot boundary.
On first launch, **Continue with Solid** signs in and loads compatible data from
the Pod without requiring a local upload. **Use this device only** keeps the
normal local operation-log backend and reveals the local workflow presets.
**Upload current data to Solid** captures the current local snapshot, validates
it, and writes supported model data to the user's Solid Pod: tasks,
projects, tags, notes, sections, issue providers, repeat configurations,
planner state, simple counters, metrics, boards, global configuration, menu
tree, active time tracking, archived tasks, archive state, and plugin data.
Device-local support data remains local, including editor drafts, sync-provider
credentials, operation-log transport metadata, the conflict journal, and the
Local REST API access token. Solid storage is separate from the normal
sync-provider pipeline.

New task resources are written as standard iCalendar `Vtodo` Things with Super
Productivity properties for fields such as the stable app ID and time tracking.
Their default write location is `super-productivity/tasks/`, but reads are not
limited to that folder: compatible tasks registered in Solid type indexes are
loaded wherever they are stored. Legacy
`https://super-productivity.com/ns#Task` resources remain supported.

Solid startup is catalog-first. Cached entities and the navigation shell can
render before profile discovery and network refresh finish. As task entities
arrive, Super Productivity rebuilds the in-memory Inbox/project ordering
indexes needed by its views; this projection repair is not written back to the
Pod. Native iCalendar tasks default to Inbox, and normalized native due dates
are included in Today unless a Super Productivity-specific due field overrides
them.

Discovery progress is durable in browser IndexedDB. Each application container
has its own non-recursive session, while compatible external iCalendar tasks use
a separate type-index session with no blind storage-root fallback. A complete
session can remove records that disappeared from its scope; interrupted or
failed sessions preserve the previously displayed slice and resume later.

Cached Solid data can be viewed offline, but the Pod remains authoritative and
there is no offline write queue. App-owned containers and external native tasks
must have proven write access before edits are accepted. Explicit and inherited
fallback-ACL grants are supported; unavailable, unknown, or rate-limited
permission results remain temporarily blocked rather than being treated as a
permanent read-only denial.
Writes are validator-bound and source-preserving: conflicts and uncertain
responses cause an exact-source refresh instead of an automatic mutation retry.
...
- A second, non-Flatpak install (AppImage, `.deb`, Snap) reads a different directory and will **not** see this data. Move between packaging formats via export/import, not by installing alongside.
- Sync or backup targets **outside** the sandbox need an explicit filesystem permission, granted with [Flatseal](https://flathub.org/apps/com.github.tchx84.Flatseal) or `flatpak override`. Without it the app reports a permission error; paths inside `~/.var/app/` always work.
...
- `SUP_LOCAL_DRAFT_*`: Device-local **crash-safe editor drafts**, one entry per in-progress edit, keyed by `SUP_LOCAL_DRAFT_${entityType}:${entityId}`. They back the fullscreen note editor so unsaved edits survive a crash or force-close, and are **never synced or uploaded** and **excluded from backups and exports** — a draft is transient device state, not application data. A draft is removed as soon as it is redundant: when the edit is saved or discarded, when the note is deleted on this device, and when the dataset is replaced wholesale (JSON import, SuperSync restore). Crash leftovers whose note is never reopened are pruned on app start once older than 14 days.
- `SUP_SOLID_DATA_LAYER_ENABLED`, `SUP_SOLID_DATA_LAYER_PRIMARY_ENABLED`, and
  `SUP_SOLID_DATA_LAYER_ISSUER`: experimental Solid backend settings. The
  enabled key allows Solid session boot/login handling; the primary key is set
  when the user continues with Solid, signs in through Solid settings, chooses
  **Use this Pod**, or completes an explicit local-to-Solid upload. It controls
  whether the app hydrates from Solid on reload. These keys are device-local.
...
- If migrating the legacy `pf` database fails, the migration dialog reports the error and waits. It also offers **Continue without old data**, which starts the app with an empty store instead of dead-ending on the error at every restart. Nothing is deleted: the `pf` database stays on disk and only gets a `_migration_skipped` marker, so migration and recovery stop picking it up. The automatic pre-migration backup can then be imported normally, and the untouched `pf` database remains a second copy for manual recovery.
