...
### Solid Data Layer (experimental)

- **Solid Pod** — An experimental native Solid backend panel. It is separate
  from the normal sync-provider setup and only appears for builds that include
  the Solid data-layer code. The panel lets you enter a Solid identity provider
  URL, enable or disable Solid boot on this device, log in or restore a session,
  log out, and explicitly upload the current local application data to an empty
  Pod. Upload validates the current local state first. After a successful
  upload, the app reloads and uses Solid as the primary data layer on this
  device.
- **Disable Solid** — Turns off Solid primary mode on this device. Local
  operation-log storage is used again after reload.
