...
The **Solid Pod** data layer is not part of this sync-provider pipeline. It uses
Solid authentication and is the primary first-run path in this build. Signing
in opens an existing Pod directly; an explicit upload is available when moving
an existing local profile into an empty Pod. Compatible
iCalendar tasks are discovered across the Pod through registered type-index
targets. It does not currently provide the same mature
conflict handling or offline queueing described for the sync providers below.
Cached Pod data remains viewable offline, while writes wait for verified access
and connectivity. The Solid runtime schedules authenticated requests per
origin, adapts concurrency after HTTP 429 responses, and resolves effective
resource/fallback-ACL permissions. It also persists scoped discovery progress,
coordinates ownership across tabs, and performs validator-bound semantic
writes. These runtime behaviors do not turn Solid
into an operation-log sync provider or add an offline outbox.
...
**Solid backend status:** Solid should still be tested with a disposable
Pod/profile while write and conflict behavior is hardened. It is a native Solid
storage backend, not a new transport inside the existing op-log sync-provider
list.
...
- **Live tracking presence (experimental, opt-in):** Enable it under Sync settings → Advanced → **Show live tracking on other devices**. While the real-time connection is active, a device that tracks time shows up on your other devices — a small chip in the header (and a silent Android notification) names the device and task, and offers a Stop button that ends the session remotely. Devices are named by platform and OS by default (e.g. “Desktop (macOS)”, “Android”); to tell two same-OS desktops apart, give each one a name under **Name of this device on other devices** (shown once the option is enabled). Idle pauses show as “Paused” without a Stop button. This is a live mirror only: it is never persisted on the server (only a short-lived in-memory slot while your devices are connected), rides the existing connection (no extra connections), and is end-to-end encrypted whenever sync encryption is enabled. SuperSync only; other providers have no real-time channel.
