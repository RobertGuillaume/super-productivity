...
The experimental **Solid Pod** data layer is not part of this sync-provider
pipeline. It uses a separate settings panel, Solid authentication, and an
explicit initial upload from the current local profile before a device hydrates
from Solid. It does not currently provide the same mature conflict handling,
offline queueing, or provider configuration flow described for the sync
providers below.
...
**Solid backend status:** Solid is experimental and should be tested with a
disposable Pod/profile. It is intended as a native Solid storage backend, not a
new transport inside the existing op-log sync-provider list.
